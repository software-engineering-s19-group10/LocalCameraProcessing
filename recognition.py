...
def fetchEmbedded():
	...
	#if(embeddedDB == None):
		#Force fetch DB
	#r = requests.get("https://boiling-reef-89836.herokuapp.com/lock_owners/api/recognition/get_embedded_data/?force=1")
	#result = r.json()
	#embeddedDB = {"encodings": result.get("encodings"), "names": result.get("names")}
	#else:
	...
	while True:
		...
		if(not(q.empty())):
			...
			for encoding in encodings:
				matches = face_recognition.compare_faces(embeddedDB["encodings"],encoding, tolerance=0.48)
				name = "Unknown"
				if True in matches:
